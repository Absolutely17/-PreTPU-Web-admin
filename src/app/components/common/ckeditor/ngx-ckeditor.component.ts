...
import {getEditorNamespace} from "./ngx-ckeditor-helper.service";
import {CKEditor4} from "./ngx-ckeditor";
...
export class NgxCkeditorComponent implements AfterViewInit, OnDestroy, ControlValueAccessor {
  /**
  ...
  /**
   * Событие выбора статьи для deepLink
   */
  @Output() articleChoose = new EventEmitter<CKEditor4.EventInfo>();

  /**
   * Событие выбора пункта меню для deepLink
   */
  @Output() menuItemChoose = new EventEmitter<CKEditor4.EventInfo>();
   ...
   */
  @Input() editorUrl = '/assets/vendor/ckeditor4/ckeditor.js';
  ...
  private subscribe(editor: any): void {
    ...
    editor.on('fileUploadRequest', evt => {
      this.ngZone.run(() => {
        this.service.fileUploadRequest(evt);
      });
    });

    editor.on('fileUploadResponse', evt => {
      this.ngZone.run(() => {
        this.service.fileUploadResponse(evt);
      });
    });
    ...
    editor.on('articleChoose', evt => {
      this.ngZone.run(() => {
        this.service.selectArticleForDeepLink(evt);
      });
    });

    editor.on('menuItemChoose', evt => {
      this.ngZone.run(() => {
        this.service.selectMenuItemForDeepLink(evt);
      })
    });
